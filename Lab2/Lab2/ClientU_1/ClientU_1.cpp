...
int _tmain(int argc, _TCHAR* argv[])
{
	...
	try
	{
		s = InitCA((char*)"192.168.43.181", (char*)"hello.txt", _wtoi(argv[1]));
		while (true)
		{
			EnterCA(s);
			int k = 0;
			std::string line;
			std::ofstream ss("hello.txt", std::ios::app);
			std::ifstream s2("hello.txt", std::ios::app);
			...
			if (k == 1) {
				for (int i = 0; i < 1; i++)
				{
					...
					if (ss.is_open())
					{
						for (int k = 1; k <= 5; k++)
						{
							ss << "(message " << k << ") Client: " << s.clientnumber << " " << asctime(timeinfo) << std::endl;
							Sleep(5000);
						}
					}
					Sleep(1000);
...
CA InitCA(char ipaddr[15], char resurce[20], int number)
{
	...
	try
	{
		...
		serv.sin_port = htons(40000); // TCP-порт 40000
